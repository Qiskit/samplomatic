...
class GroupMeasIntoBoxes(TransformationPass):
    ...
    def __init__(
        self,
        add_twirling: bool = True,
        add_basis_transform: bool = False,
        prefix_ref: str = "basis",
    ):
        TransformationPass.__init__(self)

        self.add_twirling = add_twirling
        self.add_basis_transform = add_basis_transform
    ...
    def _make_annotations(self) -> list[Annotation]:
        ...
        if self.add_twirling:
            ret.append(Twirl())

        if self.add_basis_transform:
            ret.append(
                BasisTransform(mode="measure", ref=f"{self.prefix_ref}{next(self._REF_COUNTER)}")
            )
