...
class SupportedAnnotations(str, Enum):
    """The supported values of ``annotations`` for the :class:`~.GroupMeasIntoBoxes` pass."""

    TWIRL = "twirl"
    """Add a :class:`~.Twirl` annotation."""

    BASIS_TRANSFORM = "basis_transform"
    """Add a :class:`~.BasisTransform` annotation with mode ``'measure'``."""

    ALL = "all"
    """
    Add a :class:`~.Twirl` as well as a :class:`~.BasisTransform` with mode ``'measure'``.
    """
...
class GroupMeasIntoBoxes(TransformationPass):
    ...
    Args:
        annotations: The annotations placed on the measurement boxes.
    ...
    def __init__(
        self,
        annotations: str | SupportedAnnotations = SupportedAnnotations.TWIRL,
        prefix_ref: str = "basis",
    ):
        TransformationPass.__init__(self)

        self.annotations = SupportedAnnotations(annotations)
        self.prefix_ref = prefix_ref

    def _make_annotations(self) -> list[Annotation]:
        """Make annotations for the boxes created by this pass."""
        if self.annotations == SupportedAnnotations.TWIRL:
            return [Twirl()]
        if self.annotations == SupportedAnnotations.BASIS_TRANSFORM:
