...
class AddInjectNoise(TransformationPass):
    ...
    Args:
        ...
        targets: The class of twirled-annotated boxes to target. The supported values are:

                * ``'none'`` to avoid annotating boxes of any kind.
                * ``'gates'`` to target only the boxes that contain entanglers.
                * ``'measures'`` to target only the boxes that own classical registers.
                * ``'all'`` to target boxes that contain entanglers and/or own classical
                    registers.
