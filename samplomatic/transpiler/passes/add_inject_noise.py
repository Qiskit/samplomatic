...
class AddInjectNoise(TransformationPass):
    ...
    .. note::
        Every ``ref`` is generated by appending the (truncated) hash of the box as specified by its
        :class:`~.BoxKey` to ``prefix_ref``. This means that boxes with the same :class:`~.BoxKey`
        will have the same ``ref`` assigned to them from one circuit to another. When using the
    ...
    def run(self, dag: DAGCircuit) -> DAGCircuit:
        ...
        box_to_ref: dict[BoxKey, tuple[str, str]] = {}
            ...
            if node.op.name == "box" and (twirl := get_annotation(node.op, Twirl)):
                ...
                if inject_noise_annotation := get_annotation(node.op, InjectNoise):
                    if box_key not in box_to_ref:
                        ...
                        box_to_ref[box_key] = (
                            inject_noise_annotation.ref,
                            inject_noise_annotation.site,
                        )
                    else:
                        ...
                        if self.overwrite is True:
                            ...
                            ref, site = box_to_ref[box_key]
                            node.op.annotations[annotation_idx] = InjectNoise(
                                ref, inject_noise_annotation.modifier_ref, site
                            )
                    ...
                    if box_key in box_to_ref:
                        ref, site = box_to_ref[box_key]
                    else:
                        if box_key in self._BOX_KEYS:
                            box_hash = self._BOX_KEYS[box_key]
                        else:
                            self._BOX_KEYS[box_key] = box_hash = hash(box_key)
                        ref = f"{self.prefix_ref}{str(box_hash)[:3]}"
                        site = self.site
                        box_to_ref[box_key] = ref, site
                    ...
                    node.op.annotations += [InjectNoise(ref, modifier_ref, site)]
                    # substitute back into the dag to guarantee that the
                    # change makes it back to the rust data model.
                    dag.substitute_node(node, node.op)
        return dag
