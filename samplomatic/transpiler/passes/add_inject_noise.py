...
class AddInjectNoise(TransformationPass):
    ...
    Args:
        ...
        targets: The class of twirled-annotated boxes to target. The supported values are:
                ...
                * ``'gates'`` to target boxes that contain entanglers.
                * ``'measures'`` to target boxes that own classical registers.
                * ``'all'`` to target boxes that either contain entanglers or own classical
                    registers.
