...
class AddInjectNoise(TransformationPass):
    ...
    Args:
        ...
        site: Whether to inject the noise before or after the associated hard content. Boxes
            containing a measurement or a reset always use the ``'before'`` site, regardless of
            this value; see the note below.
    ...
    .. note::
        Boxes containing a measurement or a reset are annotated with the ``'before'`` site even
        when ``site='after'``. Virtual Paulis cannot travel leftward across a measurement or a
        reset, so an injection placed after the hard content of such a box is discarded during
        :func:`~.build` on the qubits those instructions act on, while its sign is still reported.
        Forcing the ``'before'`` site guarantees that the injection is realized. Since ``site`` is
        one of the inputs to :meth:`~.InjectNoise.ref` generation, such a box is assigned the same
        ``ref`` whichever ``site`` is requested, whereas the ``ref`` of every other box changes
        along with it.
    ...
    def _get_ref(self, box_key: BoxKey, site: str) -> str:
        pair = (box_key, site)
        if pair in self._BOX_KEYS:
            return self._BOX_KEYS[pair]

        site_char = "B" if site == "before" else "A"
        for key_length in range(3, 16):
            pair_hash = f"{self.prefix_ref}{box_key.hexdigest[:key_length]}{site_char}"
    ...
    def run(self, dag: DAGCircuit) -> DAGCircuit:
        ...
        for node in dag.op_nodes():
            if node.op.name == "box" and (twirl := get_annotation(node.op, Twirl)):
                ...
                # injections cannot travel leftward across a measurement or a reset: force 'before'
                blocked = any(
                    datum.operation.name.startswith(("meas", "reset"))
                    for datum in undressed_box.body.data
                )
                site = "before" if blocked else self.site

                def make_inject_noise_annotation():
                    ref = self._get_ref(box_key, site)
                    ...
                    return InjectNoise(ref, modifier_ref, site)
