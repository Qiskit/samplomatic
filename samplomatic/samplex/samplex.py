...
class Samplex:
    ...
    * Noise model requirements that specify what types of noise models are compatible with the
    ...
    def sample(
        ...
        Args:
            samplex_input: A mapping from input names to input values, as described by :meth:`~.inputs`,
                see the guide on :doc:`guides/samplex_io`. Names that contain a period can use nested 
                dictionary expansion. Note that :class:`~.TensorInterface` is a mapping object and is therefore 
