...
class Samplex:
    r"""Represents a probability distribution over circuit parameters.

    :class:`~.Samplex` is the core type of the samplomatic library. In abstract terms, a samplex
    represents a parametric probability distribution over the parameters of some template circuit,
    as well as other array-valued fields to be used in post-processing data collected from executing
    the bound circuit. In practical terms, it implements a :meth:`~.Samplex.sample` method that
    draws from this distribution to produce random angles valid for the template circuit, along with
    the other fields. For example, a samplex instance might implement Pauli-twirling all the layers
    of a circuit. However, its semantics are not limited to this, and its structure is highly
    extensible: new group types, new distributions, new virtual gate propagation techniques
    can be added to the library.

    Internally, a samplex uses a directed acycle graph (DAG) representation that describes the
    ...
    it is allowed to act. This implies that the flow of the graph is not temporal like the DAG of a
    quantum circuit, but instead flows from nodes responsible for generating randomizations to
    nodes responsible for rendering the processed randomizations as outputs.
    ...
    * :class:`~.CollectionNode`\s are responsible reading registers and writing to output values
    ...
    Samplexes are typically generated by the :func:`~.build` function from a base circuit.
    Generating one manually is possible, though likely tedious, and the user is then also
    responsible for constructing a compatible parametric quantum circuit. The following example
    shows the simplest possible example of constructing a samplex using the :func:`~.build`
    function, and plots the resulting DAG:
