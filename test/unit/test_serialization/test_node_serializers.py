...
@pytest.mark.parametrize("tsv", ChangeBasisNodeSerializer.TSVS)
def test_change_basis_serializer_round_trip(basis_change, tsv):
    node = ChangeBasisNode("x", basis_change, "ref", 6)
    data = ChangeBasisNodeSerializer.serialize(node, tsv)
    orjson.dumps(data)
    TypeSerializer.deserialize(data)
