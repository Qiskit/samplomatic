...
class TestFindUniqueBoxInstructions:
    def test_undress_boxes_is_true(self):
    ...
    def test_undress_boxes_is_false(self):
    ...
    def test_normalize_annotations(self):
...
class TestCountUniqueBoxInstructions:
    def test_undress_boxes_is_true(self):
        """Test ``count_unique_box_instructions`` when ``undress_boxes`` is ``True``."""
        body = QuantumCircuit(4)
        body.h(0)
        body.cz(0, 1)
        annotations = [Twirl(decomposition="rzrx")]
        ...
        unique_instructions = count_unique_box_instructions([instr0, instr1, instr2, instr3])
        assert len(unique_instructions) == 3
        ...
        assert BoxKey(unique_instructions[0][0]) == BoxKey(expected_instr0)
        assert BoxKey(unique_instructions[1][0]) == BoxKey(expected_instr1)
        assert BoxKey(unique_instructions[2][0]) == BoxKey(expected_instr2)

        assert unique_instructions[0][1] == 2
        assert unique_instructions[1][1] == 1
        assert unique_instructions[2][1] == 1

    def test_undress_boxes_is_false(self):
        """Test ``count_unique_box_instructions`` when ``undress_boxes`` is ``False``."""
        body = QuantumCircuit(4)
        body.h(0)
        body.cz(0, 1)
        annotations = [Twirl(decomposition="rzrx")]
        ...
        instructions = [instr0, instr1, instr2, instr3]
        unique_instructions = count_unique_box_instructions(
            instructions,
            undress_boxes=False,
            normalize_annotations=lambda _: [],
        )
        ...
        for instr, (unique_instr, count) in zip(instructions, unique_instructions):
            expected_instr = instr
            expected_instr.operation.annotations = []
            assert BoxKey(expected_instr) == BoxKey(unique_instr)
            assert count == 1
